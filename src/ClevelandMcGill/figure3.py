    ...
    def data_to_piechart(data):
        ...
        for i, d in enumerate(data):
            ...
            if d == np.max(data):
                ...
                rr, cc = skimage.draw.line(int(np.round(END[0])), int(np.round(END[1])), int(np.round(END[0])),
                                           int(np.round(END[1])))
                piechart[rr, cc] = 1
    ...
    def data_to_piechart_aa(data):
        ...
        for i, d in enumerate(data):
            ...
            if d == np.max(data):
                ...
                theta = -(np.pi / 180.0) * (random_direction - current_angle / 2.)
                END = (50 - RADIUS / 2 * np.cos(theta), 50 - RADIUS / 2 * np.sin(theta))
                rr, cc, val = skimage.draw.line_aa(int(np.round(END[0])), int(np.round(END[1])), int(np.round(END[0])),
                                                   int(np.round(END[1])))
                piechart[rr, cc] = val
