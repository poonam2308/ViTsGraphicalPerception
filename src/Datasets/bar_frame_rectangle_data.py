...
def bf_data_generation(DATATYPE, NOISE=False, train_target=60000, val_target=20000, test_target=20000):
    if NOISE == 'True':
        NOISE = True
        SUFFIX = '_noise.'
    else:
    ...
    y_train = torch.zeros((train_target, 2), dtype=torch.float32)
    train_counter = 0

    X_val = torch.zeros((val_target, 1, 100, 100), dtype=torch.float32)
    y_val = torch.zeros((val_target, 2), dtype=torch.float32)
    val_counter = 0
...
class BarFrameRectData(Dataset):
