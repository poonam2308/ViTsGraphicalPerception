...
for i in range(len(DATATYPE_LIST)):
    ...
    vit_model.classifier = torch.nn.Linear(vit_model.config.hidden_size, args.num_classes)  # REGRESSION
    ...
    for epoch in range(args.epochs):  # Adjust the number of epochs as needed
        ...
        torch.save(vit_model.state_dict(), 'chkpt/pretrained_vit_' + DATATYPE_LIST[i] + '.pth')
    ...
    plt.savefig('trainingplots/pretrained_vit_' + DATATYPE_LIST[i] + '.png')
    plt.legend()
