...
for i in range(len(DATATYPE_LIST)):
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = wb_data_generation(DATATYPE, NOISE=args.NOISE,
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(vit_model.state_dict(), 'chkpt/pretrained_weber_vit_' + DATATYPE_LIST[i] + '.pth')
    ...
    plt.savefig('trainingplots/pretrained_weber_vit_' + DATATYPE_LIST[i] + '.png')
    plt.legend()
    MLAE = testingEpochOne_pre(vit_model, test_loader, device)
