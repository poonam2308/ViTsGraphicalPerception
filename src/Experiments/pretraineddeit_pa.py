...
from one_epoch_run import trainingEpoch_pre, validationEpoch_pre, testingEpoch_pre
...
for i in range(len(DATATYPE_LIST)):
    ...
    deit_model.classifier = torch.nn.Linear(deit_model.config.hidden_size, args.num_classes) # REGRESSION num_classes =5
    ...
    plt.legend()
    MLAE = testingEpoch_pre(deit_model, test_loader, device)
